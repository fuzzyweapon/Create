 ...
 * <p>
 * We use Quaternions for smooth rotations, cleaner code, and more efficient algorithms compared to matrices and
 * Euler angles.  There are lots of generic benefits, but unique ones that push Create to use them are the ability to
 * smoothly interpolate when a rigid body is undergoing rotation and translation (as well as simultaneously), and that
 * it is much easier and computationally efficient for accumulating angular velocity.
 */
class Quaternion {
     ...
     * Normalizes {@code this} {@link Quaternion}.
     * <p>
     * This implementation uses Padé approximant first, but if the steps are too large, it falls back to the standard,
     * but less efficient method that contains a {@link Math#sqrt(double a)}.
     * <p>
     * This makes it efficient enough to call to reduce magnitude and phase errors such as drift introduced by floating
     * point errors.
     * <p>
     * Note that normalization of a quaternion maintains its orientation but reduces its magnitude to 1.0.
     *
    ...
    final Quaternion normalized() {
        ...
        return new Quaternion(tempQuaternion.getW(), -tempQuaternion.getX(), -tempQuaternion.getY(), -tempQuaternion.getZ());
    }
    ...
    private boolean epsilonEquals(Quaternion quaternion, double epsilon) {
        if (!MathHelper.epsilonEquals(w, quaternion.w)) return false;
        if (!MathHelper.epsilonEquals(x, quaternion.x)) return false;
        if (!MathHelper.epsilonEquals(y, quaternion.y)) return false;
    ...
    private static Quaternion abs(Quaternion quaternion) {
        return new Quaternion(Math.abs(quaternion.w), Math.abs(quaternion.x), Math.abs(quaternion.y), Math.abs(quaternion.z));
    }
    ...
    Vec3d rotate(Vec3d vector) {
        ...
        Quaternion newPureQuaternion = normalized.multiply(vectorQuaternion).multiply(normalized.inverse());
        return new Vec3d(newPureQuaternion.getX(), newPureQuaternion.getY(), newPureQuaternion.getZ());
    }

    private Vec3d multiply(Vec3d part) {
        Quaternion result = new Quaternion().multiply(new Quaternion(0, part.getX(), part.getY(), part.getZ()));
